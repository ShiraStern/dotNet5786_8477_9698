...
// אזור ראשון - מתודת עזר לגישה למשתנים בקובץ הקונפיג
    ...
    public static TimeSpan GetConfigTimeSpanVal(string xmlFileName, string elemName)
    {
        ...
        TimeSpan timeSpan = root.ToTimeSpanNullable(elemName) ?? throw new FormatException($"can't convert:  {xmlFileName}, {elemName}");
        return timeSpan;
    }
    ...
    {
        XElement root = XMLTools.LoadListFromXMLElement(xmlFileName);
        string str = root.ToStringNullable(elemName) ?? throw new FormatException($"can't convert:  {xmlFileName}, {elemName}");
        return str;
    }
    ...
    {
        string? result = (string?)element.Element(name);
        return result;
    }
    #endregion
